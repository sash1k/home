...
excerpt: "Late is another easy box from HacktheBox. First, I'll need to exploit the flask application that processing the image then convert it to text with ssti (server site template injection). In this way, I can read the ssh private key and using it to gain foothold. To gain root shell, I must append the reverse shell script into one of the files that's running upon login in ssh with root privilege."
classes: wide
header:
